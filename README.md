...
**AUREVAN 不是等待提问的投资聊天机器人，而是一个持续管理专业分析工具与交易账户的本地智能助理。**

它平时在后台读取工具状态、整理跨账户结论；没有实质变化时保持安静，需要用户判断时主动汇报；涉及交易等高风险动作时，必须获得与本次任务参数绑定的明确授权，执行后返回结果并保留审计记录。

> 用户不需要每天打开多个工具逐一检查；AUREVAN 持续盯守，只在真正需要用户关注或决定时出现。

### 与普通投资 AI 的区别

| 普通投资 AI | AUREVAN |
| --- | --- |
| 等用户提问后再回答 | 持续盯守工具、账户和任务状态，必要时主动触达 |
| 依赖用户手动复制行情与持仓 | 通过 MCP / Bridge 读取已连接工具的结构化事实 |
| 一次对话通常只覆盖一个数据源 | 统一组织专业分析工具、银行现汇与 MT4/MT5 账户 |
| 模型同时承担取数、计算和解释 | 本地代码负责精确事实，模型负责归因、比较和综合分析 |
| 建议与执行相互割裂 | 高风险动作经过预检、明确授权、执行回传和审计闭环 |
| 定时轮询容易反复提醒 | 按事项状态和实质变化触发，减少重复打扰 |
...
| 工程 | macOS 桌面封装、本地数据空间、MT4/MT5 Bridge、状态驱动提醒、自动报告、MT4/MT5 受控交易协议与 322 项自动化测试 |

产品的核心不是在行情面板上增加聊天框，而是把**工具连接、任务编排、事实校验、模型分析、主动触达、人工授权、执行回传与审计**组织成一条可运行的闭环。
...
- 对涉及交易执行的动作先完成参数预检，再请求一次性明确授权；授权后由已登录的 MT4/MT5 终端执行。
...
```mermaid
flowchart LR
    T["用户提问、定时任务<br/>或状态变化"] --> A["AUREVAN 理解目标"]
    A --> S["选择 Skill<br/>组织执行步骤"]
    S --> C["MCP / Bridge<br/>调用专业工具与账户"]
    C --> V["校验来源、字段<br/>身份与数据时效"]
    V --> F["本地精确事实"]
    V --> L["模型解释、归因<br/>与跨工具综合"]
    F --> R["形成统一结论"]
    L --> R
    R --> D{"是否需要用户介入"}
    D -->|"没有实质变化"| W["保持安静<br/>继续盯守"]
    D -->|"需要关注"| N["主动汇报<br/>说明依据与影响"]
    D -->|"需要执行"| P["固定任务参数<br/>请求明确授权"]
    P --> H{"用户决定"}
    H -->|"拒绝或暂不处理"| W
    H -->|"授权"| X["重新校验最新状态"]
    X --> E["调用工具执行"]
...
- 没有实质变化时继续盯守，不为了展示“智能”而制造提醒；
- 涉及交易执行时，先通过 MT4/MT5 Bridge 预检账户、品种、方向、数量和保证金，再使用与订单指纹绑定的短时一次性授权；
- 授权后仍会重新读取最新状态。条件变化不会由系统擅自判定任务作废，而是把变化交给用户决定继续、调整或取消；系统不会自行下单或失败后自动重试。
...
MT4 与 MT5 已通过统一 Bridge 接入 AUREVAN，支持多终端、多经纪商和多账户的连接模型，并可读取账户、持仓、挂单、保证金与历史成交。两种平台均支持“订单预检 → 参数确认 → 一次性授权 → 终端执行 → 结果回传”的受控交易链路。
...
交易数据、模型密钥与工具运行数据保存在本机。AUREVAN 默认只读盯守和分析；MT4/MT5 交易只有在预检通过、订单参数固定且用户逐笔明确授权后才可执行，授权不可复用，也不会自动重试。
...
例如“检查全部账户风险”不是把一个固定模板展示给用户：AUREVAN 运行风险巡检 Skill，Skill 通过连接器读取多个账户与专业工具，统一时效和字段后，再由模型综合解释风险。工具以统一适配契约接入，助理对话与任务层保持稳定。
...
| MT4 / MT5 | 已完成统一 Bridge 接入 | 提供交易账户、持仓、挂单、保证金与历史成交；两种平台均采用预检与逐笔授权执行协议 |
...
| 是否自动下单 | 不自动下单；MT4/MT5 仅在预检后逐笔授权执行 | 投资决策、订单参数与交易授权始终由用户掌握 |
...
| 交易执行既要可用又不能越权 | 订单预检、不可变订单指纹、短时一次性授权与结果回传 | MT4/MT5 均按受控协议执行，模型、定时任务和失败重试不能绕过用户授权 |
...
- 报告生成、数据时效与跨天边界；
- 外汇实盘专业工具打包及 Web 工作区；
- MT4/MT5 账户、终端、订单预检、授权边界与命令回传链路；
- 提醒去重、状态更新和授权任务；
- 登录、访问令牌、敏感字段与本地存储；
- macOS 启动、版本识别和旧进程恢复。
...
| MT4/MT5 不是静态界面演示 | Bridge 账户发现、持仓读取、订单预检、订单指纹和一次性授权链路 |
| 交易隐私与执行具有明确边界 | 本地令牌、敏感字段裁剪、文件权限、MT4/MT5 预检和逐笔一次性授权边界测试 |
...
这是 AUREVAN 的**公开产品与工程案例仓库**，用于说明产品定位、架构决策和验证结果。示例与截图均经过脱敏处理，不包含：
