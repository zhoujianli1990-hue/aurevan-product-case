...
flowchart TB
    ...
    subgraph DEPARTMENTS["专业分析部门"]
        ...
        MARGIN["外汇保证金分析<br/>建设中"]
        NEXT["其他专业部门<br/>按标准扩展"]
    end
...
- 未完成真实适配与验收的部门不在用户界面显示，也不使用模拟数据伪装接入；
...
用户问：“黄金上涨会不会影响我的保证金账户和银行外汇持仓？”

当相关部门均已接入后，AUREVAN 的处理方式是：

1. 调用黄金分析部门，取得上涨原因、趋势和风险因子；
2. 调用保证金分析部门，检查黄金及美元相关仓位；
3. 调用外汇实盘部门，检查银行现汇持仓的美元敞口；
4. 对来源和数据时间进行校验；
5. 由 AUREVAN 汇总为一份面向用户的影响分析。

示例用于解释架构组合方式；当前公开版本只把已接入并验证的部门列为已实现能力。
